...
for n_phi=1:N_files
    ...
    %% Calculate for mean wavelength first
    if n_phi==4
        I_k_1=zeros(h_grid_N,theta_grid_N);
        ...
        for n_point=1:N_points
            ...
            if ~isempty(Boyao_ind)
                ...
                I_x_temp=normpdf(k_X_k*((2*pi)/lambda),k_out(n_point,1),peak_width);
                I_y_temp=normpdf(k_Y_k*((2*pi)/lambda),-k_out(n_point,2),peak_width);

                I_k_1=I_k_1+peak_I*(I_y_temp.*I_x_temp).*abs(J_sim);
    ...
    I_tot_real=I_tot.*abs(J_sim);
    I_conv_k=conv2(I_tot_real,det_response,"same")./abs(J_sim);
    ...
    %Plot symmetry directions
    hold on
    R=[cosd(phi_in+45), -sind(phi_in+45);sind(phi_in+45) cosd(phi_in+45)];
    x_lines=[0 -4];
    y_lines=[0 0];
    ...
    R2=[cosd(phi_in-45), -sind(phi_in-45);sind(phi_in-45) cosd(phi_in-45)];

    pos2_rotated=R2*pos;

    plot(pos1_rotated(1,:)+k_mag/(1e10*sqrt(2)), pos1_rotated(2,:), 'LineWidth',1,'Color','k')
    plot(pos2_rotated(1,:)+k_mag/(1e10*sqrt(2)), pos2_rotated(2,:), 'LineWidth',1,'Color','k')
...
%% Create data slices for comparison
...
I_cut_exp_norm=(I_cut_exp)./(max(I_cut_exp));
k_exp_cut=k_X2(:,row_ind_exp)/1e10;

plot(k_exp_cut,I_cut_exp_norm,'LineWidth',1,'Color',plt_line_colour)
...
fig_h_w1=figure;
imagesc(theta_grid_vec,h_grid_vec,I_k_1'.*abs(J_sim))
ylabel('h/mm','fontname',plot_font,'fontsize',axes_font_size)
xlabel('\phi/^\circ','fontname',plot_font,'fontsize',axes_font_size)
ax = gca;
ax.XAxis.FontSize = tick_font_size;
ax.YAxis.FontSize = tick_font_size;
set(gca,'LineWidth',2.5,'YDir','normal');

fig_h_w2=figure;
imagesc(theta_grid_vec,h_grid_vec,I_k'.*abs(J_sim))
ylabel('h/mm','fontname',plot_font,'fontsize',axes_font_size)
xlabel('\phi/^\circ','fontname',plot_font,'fontsize',axes_font_size)
ax = gca;
ax.XAxis.FontSize = tick_font_size;
...
fig_h_w3=figure; 
imagesc(theta_grid_vec,h_grid_vec,I_conv_k'.*abs(J_sim))
ylabel('h/mm','fontname',plot_font,'fontsize',axes_font_size)
xlabel('\phi/^\circ','fontname',plot_font,'fontsize',axes_font_size)
ax = gca;
ax.XAxis.FontSize = tick_font_size;
...
% exportgraphics(fig_h,'../LiF_sim_comp_v2.png','Resolution',1000)
% exportgraphics(fig_h2,'../LiF_sim_comp_pt2_v2.png','Resolution',1000)
% exportgraphics(fig_h2,'../LiF_sim_comp_pt2_v2.eps')
% exportgraphics(fig_h,'../LiF_sim_comp_v2.pdf','ContentType','vector')

% exportgraphics(fig_h_w1,'../Cylinder_Fig6_a.pdf','ContentType','vector')
% exportgraphics(fig_h_w2,'../Cylinder_Fig6_b.pdf','ContentType','vector')
% exportgraphics(fig_h_w3,'../Cylinder_Fig6_c.pdf','ContentType','vector')
